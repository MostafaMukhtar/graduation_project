...
uint8_t tx_buf[4][AX_25_FRAME_LEN];//edit 4--3
...
uint8_t sf=0,sl=3,rf=0,rl=2,s=2,r=0; //edit
uint8_t VS=0,VR=0;
...
void ax_25_make_I_frame(void){            
    ...
    ax_25_set_address_field(comm_addr,gnd_addr,SSID_NAME);
    ...
    tx_buf_pointer++;
    tx_buf_size++;
    if(tx_buf_size==4) tx_buf_busy=1;
    if(tx_buf_pointer>3) tx_buf_pointer=0;
...
uint8_t Check_distnation(void){
    int i=1;
    for(;i<7;i++){
      if(gnd_addr[i-1]!=(received_ax_25_frame[i]>>1)){
        return 0 ;}
      }
...
uint8_t Check_Source(void){
    ...
    for(;i<14;i++)
    {
    if(comm_addr[i-8]!=(received_ax_25_frame[i]>>1))
    {
        return 0;
    }
    }
...
void Srej_Condtion(uint8_t nr){//edit
 //VS--;
  if(nr<=3){
    ...
    memcpy(tx_frame,tx_buf[nr-4],AX_25_FRAME_LEN);
    tx_buffer_flag=1;}
}
